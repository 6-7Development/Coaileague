...
- **Core Feature Areas**:
    ...
    - **DispatchOS™ - Computer-Aided Dispatch**: **NEW November 2025** - Real-time CAD system for emergency services dominance. Includes GPS tracking with 10-second updates, live map view of all units with color-coded status indicators (available, en route, on scene, offline), incident queue with priority-based assignment (emergency/urgent/routine), mobile unit interface for accepting calls and status updates, dispatcher command center with incident/unit management, and timeline tracking (call received → dispatched → en route → arrived → cleared). Integrates with ScheduleOS™ (only dispatch on-shift units), TimeOS™ (auto clock-in at scene), and BillOS™ (incident time → invoice). Database tables: gps_locations (enhanced for dispatch tracking), dispatch_incidents, dispatch_assignments, unit_statuses, dispatch_logs. Supports certifications, zones, equipment assignment, and performance metrics (response time, travel time, scene time).
