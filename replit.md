...
- **Gemini Agent Mascot (Dec 2, 2025):** Emotional twin-star Gemini AI mascot rendered as draggable 200px (expanded 300px) interactive bubble. Globally visible on ALL pages (public and authenticated). Features: state-reactive behaviors, persistent positioning via localStorage, drag controls with smooth positioning (positive coordinates inverted delta), expand/minimize/reset buttons, and tooltip guidance. Fixed critical issue by moving MascotRenderer outside ProtectedRoute to ensure visibility on public pages.
