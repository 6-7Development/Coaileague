...
// ── Wave 21C — Generated Documents Registry ───────────────────────────────────
// Every PDF generated by the engine is logged here for legal retention.
// Lighter than document_vault — focuses on generation audit trail.
export const generatedDocuments = pgTable("generated_documents", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  workspaceId: varchar("workspace_id").notNull(),
  // Type: 'uof_report' | 'dar' | 'dps_audit_packet' | 'pay_stub' | 'offer_letter' | 'incident_report'
  documentType: varchar("document_type", { length: 80 }).notNull(),
  // Reference to source record (incidentId, darId, etc.)
  referenceId: varchar("reference_id", { length: 200 }),
  // Cryptographic document ID — appears in footer, used for verification
  docId: varchar("doc_id", { length: 80 }).notNull().unique(),
  // Where the PDF lives (relative path or full URL)
  storagePath: text("storage_path"),
  fileSizeBytes: integer("file_size_bytes"),
  pageCount: integer("page_count"),
  // Who triggered generation
  generatedBy: varchar("generated_by"),
  generatedByName: varchar("generated_by_name", { length: 200 }),
  // State at generation time — for reproducibility and legal integrity
  snapshotJson: jsonb("snapshot_json"),
  // RKE citations embedded in this doc
  ...
  index("gendoc_workspace_type_idx").on(t.workspaceId, t.documentType),
  index("gendoc_ref_idx").on(t.referenceId),
  index("gendoc_docid_idx").on(t.docId),
]);
