...
import { MessageSquare, X, ArrowRight, CalendarDays, CheckSquare, Users } from "lucide-react";
...
function getContextPanel(path: string): { greeting: string; insight: string; chips: Chip[] } {
  if (path.startsWith("/time-tracking") || path.startsWith("/clock")) {
    return {
      ...
      chips: [
        ...
        { label: "GPS not working?", icon: MessageSquare, prompt: "My GPS isn't capturing. What should I do?" },
      ],
    };
  }
  if (path.startsWith("/scheduling") || path.startsWith("/schedule")) {
    return {
      ...
      chips: [
        ...
        { label: "Explain schedule", icon: MessageSquare, prompt: "Explain the current schedule setup and any issues you notice." },
      ],
    };
  }
  ...
  if (path.startsWith("/timesheets")) {
    return {
      ...
      chips: [
        ...
        { label: "Explain my hours", icon: MessageSquare, prompt: "Break down my timesheet for the current pay period." },
      ],
    };
  }
  if (path.startsWith("/payroll")) {
    return {
      ...
      chips: [
        { label: "How calculated?", icon: MessageSquare, prompt: "Walk me through how payroll is being calculated for this period." },
  ...
  if (path.startsWith("/compliance")) {
    return {
      ...
      chips: [
        ...
        { label: "Explain requirement", icon: MessageSquare, prompt: "Explain what this compliance requirement means and how to complete it." },
      ],
    };
  }
  if (path.startsWith("/billing") || path.startsWith("/invoices")) {
    return {
      ...
      chips: [
        { label: "Explain this charge", icon: MessageSquare, prompt: "Explain the current billing charges and what they cover." },
  ...
  return {
    ...
    chips: [
      { label: "What can you do?", icon: MessageSquare, prompt: "What can you help me with on this page and throughout the platform?" },
