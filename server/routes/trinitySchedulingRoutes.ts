...
router.get('/pending-approvals', requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    // Schema export is optional — the table may not have shipped yet — so
    // resolve it via `any` and gate on a runtime null check below.
    const trinitySchema = await import('@shared/schema').catch(() => ({} as any)) as any;
    const trinityProposedActions: any = trinitySchema.trinityProposedActions ?? null;
