...
import { requireStarter } from "../tierGuards";
import {  } from '../services/financialCalculator';
import {
  workspaces,
  shifts,
  employees,
  users,
  platformRoles} from "@shared/schema";
...
uter.get('/ai/status', requireAuth, async (req: any, res) => {
    try {
...
uter.post('/start-trial', requireAuth, async (req: any, res) => {
    try {
...
outer.post('/activate', requireManager, async (req: any, res) => {
    try {
      const userId = req.user?.id || req.user?.claims?.sub;
...
outer.get('/status', async (req: any, res) => {
    try {
      let userId: string;
      let user: any;
...
outer.post('/acknowledge/:shiftId', requireAuth, async (req: any, res) => {
    try {
      const userId = req.user?.id || req.user?.claims?.sub;
    ...
    try {
      ...
      await db.transaction(async (tx) => {
        ...
        await tx.update(workspaces).set({
          ...
          scheduleosActivatedBy: enabled ? userId : null}).where(eq(workspaces.id, workspaceId));
        ...
        await db.transaction(async (tx) => {
          for (const assignment of aiResponse.assignments) {
            await tx.update(shifts).set({
              ...
              status: 'scheduled',
              updatedAt: new Date()}).where(eq(shifts.id, assignment.shiftId));
    ...
    try {
      ...
      await db.transaction(async (tx) => {
        ...
        await tx.update(scheduleProposals).set({
          ...
          shiftIdsCreated,
          updatedAt: new Date()}).where(eq(scheduleProposals.id, id));
