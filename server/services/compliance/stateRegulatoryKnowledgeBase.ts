...
export async function getStateConfig(stateCode: string): Promise<StateRegulatoryConfig | null> {
  ...
  return {
    ...
    licenseTypes: ((row as Record<string, unknown>).licenseTypes as any[]) ?? [],
    keyStatutes: ((row as Record<string, unknown>).keyStatutes as any[]) ?? [],
    minimumInsuranceCoverage: ((row as Record<string, unknown>).minimumInsuranceCoverage as any[]) ?? [],
    ...
    hardBlockRules: ((row as Record<string, unknown>).hardBlockRules as any[]) ?? [],
